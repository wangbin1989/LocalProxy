...
public sealed class TcpForwardHandler : ITunnelHandler
{
    private readonly ProxyConfig _config;
    private readonly ILogger<TcpForwardHandler> _logger;
    ...
    public TcpForwardHandler(ProxyConfig config, ILogger<TcpForwardHandler> logger)
    {
        _config = config;
        _logger = logger;
    ...
    public async Task StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation(
            "TCP tunnel '{Name}' started on port {LocalPort} -> {RemoteHost}:{RemotePort}",
    ...
    public async Task StopAsync(CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Stopping TCP tunnel '{Name}'", Name);
        ...
        _logger.LogInformation(
            "TCP tunnel '{Name}' stopped. Total: {BytesIn} bytes in, {BytesOut} bytes out",
    ...
    private async Task HandleClientAsync(TcpClient localClient)
    {
        ...
        {
            localClient.Dispose();
            _logger.LogWarning("TCP tunnel '{Name}': connection limit reached, rejecting", Name);
            return;
        }
        ...
        {
            _logger.LogDebug("TCP tunnel '{Name}': connection error: {Message}", Name, ex.Message);
        }
